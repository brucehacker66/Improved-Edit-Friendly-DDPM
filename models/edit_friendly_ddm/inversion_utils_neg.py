"""
Modified inversion utilities with negative prompt support

This extends the original inversion_utils.py to support negative prompting,
which helps clear semantic space from the source concept to make room for the target.
"""
...
def inversion_reverse_process_neg(model,
                                  xT,
                                  etas=0,
                                  prompts="",
                                  ...
                                  asyrp=False):
    """
    Modified reverse process with negative prompt support
    ...
    Args:
        ...
        negative_prompts: List of negative prompts (can be source prompt to clear semantic space)
    ...
    negative_embeddings = encode_text(model, negative_prompts)
    ...
    for t in op:
        ...
        ## Negative embedding (instead of unconditional)
        with torch.no_grad():
            neg_out = model.unet.forward(xt, timestep=t,
        ...
        if prompts:
            ...
            noise_pred = neg_out.sample + cfg_scales_tensor * (cond_out.sample - neg_out.sample)
        else:
            noise_pred = neg_out.sample
