"""
Edit-Friendly DDPM + P2P with Dynamic PSD Adjustment

Improvement implemented:
- Applies dampening factor to the extracted noise during editing pass
- Weakens the structural "imprint" from the edit-friendly noise
- Allows the text prompt to manifest more clearly

Strategy: Multiply the noise variance by a factor (0.8-0.95) during editing,
reducing the constraint from the inverted noise trajectory.
"""
...
def edit_image_psd(
    ...
    """
    Edit image with dynamic PSD adjustment

    Args:
        edit_method: Which PSD configuration to use
            - 'psd-0.95': 5% noise reduction
            - 'psd-0.90': 10% noise reduction
            - 'psd-0.85': 15% noise reduction
            - 'psd-0.80': 20% noise reduction
            - 'psd-combined': PSD + other optimizations
    ...
    # Configure PSD dampening based on method
    psd_configs = {
        "psd-0.95": {"damping": 0.95, "cross": 0.4, "self": 0.6},
        "psd-0.90": {"damping": 0.90, "cross": 0.4, "self": 0.6},
        "psd-0.85": {"damping": 0.85, "cross": 0.4, "self": 0.6},
        "psd-0.80": {"damping": 0.80, "cross": 0.4, "self": 0.6},
        "psd-combined": {"damping": 0.90, "cross": 0.3, "self": 0.7},  # Combined with relaxed cross-attention
    }

    if edit_method not in psd_configs:
        raise NotImplementedError(f"No edit method named {edit_method}")

    config = psd_configs[edit_method]
    psd_damping = config["damping"]
    cross_steps = config["cross"]
    self_steps = config["self"]

    # Editing with P2P and PSD dampening
    ...
    w0, _ = inversion_reverse_process_psd(
        ...
        psd_damping_factor=psd_damping
    )
    ...
    image_instruct = txt_draw(
        ...
        f"PSD Damping: {psd_damping}, Cross: {cross_steps}, Self: {self_steps}\\n"
        f"source: {prompt_src}\\ntarget: {prompt_tar}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--edit_method_list', nargs='+', type=str, default=["psd-0.90"])
    ...
    for i, image_file in enumerate(image_files, 1):
        ...
        for edit_method in edit_method_list:
            ...
            try:
                ...
                concatenated_image, edited_image = edit_image_psd(
